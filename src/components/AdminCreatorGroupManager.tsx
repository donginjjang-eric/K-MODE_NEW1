...
export default function AdminCreatorGroupManager({ group, creators, groups }: {
  ...
  async function run(action: () => Promise<void>, success: string, section: "settings" | "members" | "agency") {
    const setSectionMessage = section === "settings" ? setSettingsMessage : section === "members" ? setMemberMessage : setAgencyMessage;
    setBusy(true);
    setSectionMessage("");
    try {
      await action();
      setSectionMessage(success);
      setSelectedAvailable(new Set());
      setSelectedMembers(new Set());
      router.refresh();
    } catch (error) {
      setSectionMessage(error instanceof Error ? error.message : "작업을 완료하지 못했습니다.");
    } finally {
      setBusy(false);
    }
  }
    ...
    <div className="admin-group-detail">
      ...
      <section className="admin-detail-section" aria-labelledby="group-members-heading"><div className="admin-section-heading"><div><p>MEMBERS</p><h2 id="group-members-heading">구성원 관리</h2></div><span>{group.creators.length}명</span></div>
        <div className="admin-member-columns">
          <div><h3>현재 구성원</h3><div className="admin-member-list">{group.creators.length ? group.creators.map((creator) => <label key={creator.id}><input type="checkbox" checked={selectedMembers.has(creator.id)} onChange={() => toggle(setSelectedMembers, creator.id)} /><span><strong>{creator.display_name}</strong><small>{numberFormat.format(creator.followerTotal)} 팔로워 · {creator.market}</small></span></label>) : <p className="admin-factual-empty">배정된 크리에이터가 없습니다.</p>}</div><div className="admin-member-actions"><button className="st-btn light" type="button" disabled={busy || !selectedMembers.size} onClick={() => run(() => memberPatch(group.id, "remove", [...selectedMembers]), "선택한 구성원을 제거했습니다.", "members")}>구성원 제거</button><select aria-label="이동할 관리 그룹" value={moveTarget} onChange={(event) => setMoveTarget(event.target.value)}><option value="">이동할 그룹</option>{groups.filter((item) => item.id !== group.id && item.status === "active").map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select><button className="st-btn" type="button" disabled={busy || !selectedMembers.size || !moveTarget} onClick={() => run(() => memberPatch(moveTarget, "assign", [...selectedMembers]), "선택한 구성원을 다른 그룹으로 이동했습니다.", "members")}>그룹 이동</button></div></div>
          <div><h3>구성원 배정</h3><input className="admin-member-search" type="search" value={availableSearch} onChange={(event) => setAvailableSearch(event.target.value)} placeholder="이름 또는 핸들 검색" aria-label="배정할 크리에이터 검색" /><div className="admin-member-list">{available.slice(0, 30).map((creator) => <label key={creator.id}><input type="checkbox" checked={selectedAvailable.has(creator.id)} onChange={() => toggle(setSelectedAvailable, creator.id)} /><span><strong>{creator.display_name}</strong><small>{creator.managementGroupName ? `${creator.managementGroupName}에서 이동` : "미지정"} · {creator.market}</small></span></label>)}</div><button className="st-btn dark" type="button" disabled={busy || !selectedAvailable.size || group.status === "inactive"} onClick={() => run(() => memberPatch(group.id, "assign", [...selectedAvailable]), "선택한 크리에이터를 배정했습니다.", "members")}>선택 구성원 배정</button></div>
        </div>
        {memberMessage ? <p className="admin-operation-message" role="status">{memberMessage}</p> : null}
      </section>

      <section className="admin-detail-section" aria-labelledby="group-agency-heading"><div className="admin-section-heading"><div><p>AGENCY ACCESS</p><h2 id="group-agency-heading">대행사 접근 관리</h2></div><span>소속 캠페인·거래·정산 조회 전용</span></div><div className="admin-agency-invite"><label><span>대행사 이메일 초대</span><input type="email" value={agencyEmail} onChange={(event) => setAgencyEmail(event.target.value)} placeholder="agency@example.com" /></label><button className="st-btn dark" type="button" disabled={busy || !agencyEmail.trim()} onClick={() => run(() => api(`/api/admin/creator-groups/${group.id}/agency-users`, { method: "POST", body: JSON.stringify({ email: agencyEmail }) }).then(() => { setAgencyEmail(""); }), "대행사 이메일을 초대했습니다.", "agency")}>초대</button></div><div className="admin-agency-list">{group.agencyUsers.length ? group.agencyUsers.map((user) => <div key={`${user.email}-${user.status}`}><span><strong>{user.email}</strong><small>{user.status}</small></span><button className="st-btn light" type="button" disabled={busy || user.status === "revoked"} onClick={() => run(() => api(`/api/admin/creator-groups/${group.id}/agency-users`, { method: "DELETE", body: JSON.stringify({ email: user.email }) }).then(() => undefined), "대행사 연결을 해제했습니다.", "agency")}>대행사 연결 해제</button></div>) : <p className="admin-factual-empty">초대된 대행사 계정이 없습니다.</p>}</div>{agencyMessage ? <p className="admin-operation-message" role="status">{agencyMessage}</p> : null}</section>
