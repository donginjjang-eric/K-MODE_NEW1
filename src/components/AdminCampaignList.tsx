  ...
  return (
    <section className="admin-campaign-list">
      ...
      {campaigns.length ? <div className="admin-campaign-table-wrap"><table><thead><tr><th scope="col">title</th><th scope="col">category</th><th scope="col">markets</th><th scope="col">platforms</th><th scope="col">applications</th><th scope="col">matched</th><th scope="col">slots</th><th scope="col">deadline</th><th scope="col">status</th><th scope="col">actions</th></tr></thead><tbody>
        {campaigns.map((campaign) => <tr key={campaign.id}><td><Link href={`/dashboard/admin/campaigns/${campaign.id}`}>{campaign.title}</Link></td><td>{campaign.category}</td><td>{campaign.markets.join(", ")}</td><td>{campaign.platforms.join(", ")}</td><td>{campaign.application_count}</td><td>{campaign.matched_count}</td><td>{campaign.slots}</td><td>{campaign.application_deadline ? new Intl.DateTimeFormat("ko-KR", { dateStyle: "medium" }).format(new Date(campaign.application_deadline)) : "-"}</td><td><span className={`admin-campaign-status is-${campaign.status}`}>{STATUS_LABELS[campaign.status]}</span></td><td><AdminCampaignListActions campaignId={campaign.id} status={campaign.status} /></td></tr>)}
