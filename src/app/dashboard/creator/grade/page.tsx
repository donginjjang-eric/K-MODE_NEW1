...
const GRADES = [
  { name: "STARTER", label: "시작", condition: "첫 캠페인을 준비하는 단계", minimum: 0 },
  { name: "RISING", label: "성장", condition: "완료 캠페인 1~2건", minimum: 1 },
  { name: "PRO", label: "전문가", condition: "완료 캠페인 3건 이상", minimum: 3 },
];
  ...
  return (
    <div className="creator-campaigns-page">
      <header className="creator-page-heading creator-page-heading-wide">
        <p>크리에이터 등급</p><h1>등급</h1>
        <span>한국 브랜드와의 협업 완료 이력에 따라 더 많은 캠페인과 판매 기회가 열립니다.</span>
      </header>
      <section className="creator-grade-hero">
        <div><p>현재 등급</p><strong>{GRADES.find((grade) => grade.name === current)?.label}</strong><span><span>완료 캠페인</span> {completed}<span>건</span></span></div>
        <div>{current === "PRO"
      ...
      <section className="creator-grade-grid" aria-label="크리에이터 등급 안내">
        {GRADES.map((grade) => <article key={grade.name} className={grade.name === current ? "is-current" : ""}><span>{String(grade.minimum).padStart(2, "0")}</span><h2>{grade.label}</h2><p>{grade.condition}</p>{grade.name === current ? <b>현재 등급</b> : null}</article>)}
      </section>
    </div>
  );
}
