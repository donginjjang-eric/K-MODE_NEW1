...
export default async function AdminCampaignDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <div className="admin-campaign-detail">
    ...
    <header className="admin-campaign-page-head"><div><p>{campaign.category}</p><h1 className="st-title">{campaign.title}</h1><p className="st-sub">{campaign.brief}</p></div><Link className="st-btn" href={`/dashboard/admin/campaigns/${campaign.id}/edit`}>Edit campaign</Link></header>
