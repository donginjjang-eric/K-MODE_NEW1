...
import { getCreatorAccountForUser, getDesignerForUser } from "@/lib/db";
...
export async function GET() {
  ...
  try {
    const row = await getCreatorAccountForUser(user.id);
    if (row) creator = { id: row.id, displayName: row.display_name, approvalStatus: row.approval_status };
  } catch {
    // 조회 실패 시 크리에이터 정보 없이 반환
  }

  return Response.json({ ok: true, user, designer, creator });
}
