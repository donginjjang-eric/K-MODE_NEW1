...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!id || typeof action !== "string" || !VALID_ACTIONS.has(action as AdminParticipationAction) || (note !== undefined && typeof note !== "string")) {
    return Response.json({ code: "invalid_request", error: "참여 상태와 메모를 확인해 주세요." }, { status: 400 });
  }
  try {
    const participation = await transitionBeautyPartnerParticipation(designer.id, user.id, id, action as AdminParticipationAction, note ?? "");
